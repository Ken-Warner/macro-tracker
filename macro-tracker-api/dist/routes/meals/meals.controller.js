...
async function createNewMeal(req, res) {
    ...
    try {
        ...
        const uuid = await log(loggingLevels.ERROR, `createNewMeal: ${e.message}`, req.body);
...
async function createNewMealRaw(req, res) {
    ...
    try {
        ...
        const uuid = await log(loggingLevels.ERROR, `createNewMealRaw: ${e.message}`, req.body);
        res.status(500).send(formatResponse(uuid));
    }
}
async function getMealHistory(req, res) {
    ...
    if (!validator.isValidDate(req.query.fromDate) ||
    ...
    try {
        const mealHistoryWithoutRecurring = await getMealHistoryWithRange(req.session.userId, req.query.fromDate, req.query.toDate);
        let newMeals = [];
        let newMacroTotals = [];
        ...
        for (let meal of mealHistory) {
            ...
            let deepMealHistoryDate = deepMealHistory.find((el) => el.mealsDate === meal.date);
        ...
        const uuid = await log(loggingLevels.ERROR, `getMealHistory: ${e.message}`, req.query);
        res.status(500).send(formatResponse(uuid));
    }
}
async function getMeals(req, res) {
    ...
    try {
        ...
        const uuid = await log(loggingLevels.ERROR, `getMeals: ${e.message}`, req.query);
        res.status(500).send(formatResponse(uuid));
    }
}
async function deleteMealById(req, res) {
    ...
    try {
        ...
        const uuid = await log(loggingLevels.ERROR, `deleteMealById: ${e.message}`, { userId: req.session.userId, requestParamaters: req.params });
        res.status(500).send(formatResponse(uuid));
    }
}
async function putMealIsRecurring(req, res) {
    ...
    try {
        ...
        const uuid = await log(loggingLevels.ERROR, `putMealIsRecurring: ${e.message}`, { userId: req.session.userId, requestParamaters: req.params });
        res.status(500).send(formatResponse(uuid));
    }
}
