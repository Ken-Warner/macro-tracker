...
async function getMacrosFromDateRange(req, res) {
    ...
    try {
        let macroData = await selectMacrosFromDateRange(req.session.userId, req.query.fromDate, req.query.toDate);
        ...
        const uuid = await log(loggingLevels.ERROR, `getMacrosFromDateRange: ${e.message}`, req.query);
...
async function getTodaysMacros(req, res) {
    ...
    try {
        ...
        const uuid = await log(loggingLevels.ERROR, `getTodaysMacros: ${e.message}`, req.query);
        res.status(500).send(formatResponse(uuid));
    }
}
